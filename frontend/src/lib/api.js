...
export const authService = {
  ...
  logout: () => {
    localStorage.removeItem('token');
    if (typeof window !== 'undefined' && chatService.socket) {
...
// ---- Chat services ----
// The real socket is created lazily on the client (see ChatSocket).
// `chatService.socket` is a safe placeholder: client code that touches it
// (emit, auth, connect, disconnect) gets a real socket at runtime once the
// client-only ChatSocket has initialized it; on the server it is a no-op.
